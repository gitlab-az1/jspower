 ...
 * @example
 * ```ts
 * import type { ArrayIndices, ArrayValues } from 'typesdk/types';
 *
...
/**
 * Provides valid values for a constant array or tuple.
 * This is the same as `Element[number]` but works with readonly arrays and tuples.
 * 
 * @example
 * ```ts
 * import type { ArrayValues } from 'typesdk/types';
 * 
 * const weekdays = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'] as const;
 * 
 * type WeekdayName = ArrayValues<typeof weekdays>;
 * 
 * const getWeekdayName = (day: number): WeekdayName => weekdays[day];
 * 
 * getWeekdayName(0); // 'Monday'
 * 
 * getWeekdayName(7); // Error: Argument of type '7' is not assignable to parameter of type '0 | 1 | 2 | 3 | 4 | 5 | 6'.
 * ```
 */
...
/**
 * Get the entries of a type that can be iterated.
 */
...
/**
 * A schema is a type that describes the structure of an object.
 */
export type Schema<ObjectType, ValueType> = ObjectType extends string
  ...
  ? ValueType
  : ObjectType extends (...args: unknown[]) => unknown
  ? ValueType
...
*/
export type SchemaObject<ObjectType extends object, K> = {
...
/**
 * All of node's typed array types
 */
...
/**
 * A unknown array or tuple
 */
export type UnknownArrayOrTuple = readonly [...unknown[]];

/**
 * Roman alphabet characters in upper case.
 */
