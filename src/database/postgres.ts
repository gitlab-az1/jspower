...
import { Deferred, asyncRetry } from '../async';
...
export class Database {
  ...
  constructor(connection: string | PostgresConnectionProps) {
    this.#config = {
...
export async function connect(): Promise<Database> {
  ...
  if(database && (await database.isOnline())) return database;
  database = new Database(process.env.TYPESDK_DEFAULTS_POSTGRES_CONNECTION_STRING!);
