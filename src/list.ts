import { validateSerializableObjectToJSON } from './validators';
import { validateCRC32Table } from './algorithms/crc32';
 ...
 * @param _key The string to hash
 * @param _max_buckets The maximum number of buckets to return
 * @returns The CRC32 hash of the string
 */
function hashCode(_key: string, _max_buckets?: number): number {
  validateCRC32Table(_ucrc_32_table);
  let crc = 0xFFFFFFFF;

  for(let i = 0; i < _key.length; i++) {
    crc = (crc >>> 8) ^ _ucrc_32_table[(crc ^ _key.charCodeAt(i)) & 0xFF];
  }

  const value = (crc ^ 0xFFFFFFFF) >>> 0;

  if(!_max_buckets || typeof _max_buckets !== 'number' || _max_buckets < 1) return value;
  return math.floor(value % _max_buckets);
}


/**
 * Definition of a storage block used in the list.
 */
export type StorageBlock<T> = {
  readonly key: string;
  readonly code: number;
  readonly value: T;
  next: StorageBlock<T> | null;
};


/**
 * Definition of a readonly storage block used in the list.
 */
export type ReadonlyStorageBlock<T> = StorageBlock<T> & {
  readonly next: ReadonlyStorageBlock<T> | null;
};


/**
 * Represents a linked list that can store elements of type T.
 * @typeparam T The type of elements stored in the list.
 */
export class List<T> {
  readonly #maxBuckets: number;
  readonly #storage: Array<StorageBlock<T>>;

  /**
   * Constructor for the List class.
   * @param length The optional length used to initialize the list with a specific size.
  ...
  constructor(length?: number) {
    ...
    this.#maxBuckets = length ?? 10;
    this.#storage = new Array<StorageBlock<T>>(this.#maxBuckets);
  }
  ...
  public get length(): number {
    ...
    for(let i = 0; i < this.#storage.length; i++) {
      ...
      let current: StorageBlock<T> | null = this.#storage[i];
  ...
  #addItem(value: T, _key?: string): void {
    if(!_key && !validateSerializableObjectToJSON(value)) {
      throw new Exception('The provided value must be serializable if no key is provided');
    }

    const k = _key ?? JSON.stringify(value);
    const index = hashCode(k, this.#maxBuckets);

    const node = this.#createNode(k, value);

    if(!this.#storage[index]
      || typeof this.#storage[index] === 'undefined'
   ...
   * @throws Exception if the provided value is not serializable and no key is provided.
   */
  public push(value: T, key?: string): void {
    this.#addItem(value, key);
  }

  #createNode(key: string, value: any): StorageBlock<T> {
    return {
      key,
      code: hashCode(key, this.#maxBuckets),
      value,
      next: null,
    };
  }
  ...
  /**
   * Look up an item in the list by key.
   * @param key The key to look up.
   * @returns The value associated with the key, or null if not found.
   */
  public lookup(key: string): T | null {
    const index = hashCode(key, this.#maxBuckets);
    if(!this.#storage[index]) return null;

    let current: StorageBlock<T> | null = this.#storage[index];
  ...
  #removeItem(key: string): boolean {
    ...
    let current: StorageBlock<T> | null = this.#storage[index];
    let previous: StorageBlock<T> | null = null;
  ...
  /**
   * Get the storage block at a specific index.
   * @param index The index of the storage block to retrieve.
   * @returns The storage block at the specified index, or null if the index is out of bounds.
   */
  public at(index: number): ReadonlyStorageBlock<T> | null {
    if(index < 0 || index > this.#maxBuckets) return null;
  ...
  #Tree(): ReadonlyStorageBlock<T> {
    ...
    for(let i = 0; i < this.#storage.length; i++) {
      ...
      while(current) {
        const node = this.#createNode(current.key, current.value);
        root.next = node;
        root = root.next;
        current = current.next;
      }
  ...
  #Reverse() {
    for(let i = 0; i < this.#storage.length; i++) {
      ...
      let current: StorageBlock<T> | null = this.#storage[i];
      let previous: StorageBlock<T> | null = null;
      let next: StorageBlock<T> | null = null;
